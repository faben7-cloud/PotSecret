...
export default async function PublicPotSuccessPage({
  ...
  const { data: contribution } = sessionId
    ? await admin
        .from("contributions")
        .select(
          "status, amount, currency, contributor_display_name, is_anonymous"
        )
  ...
  const contributorName = contribution?.is_anonymous
    ...
    : contribution?.contributor_display_name ||
      session?.metadata?.contributor_display_name ||
      null;
  ...
  const safeEventType =
    String(pot.event_type) as keyof typeof eventTypeLabel;

  const safeEventTypeLabel =
    eventTypeLabel[safeEventType] ?? "Événement";
      ...
      <div className="w-full rounded-[2rem] border border-white/60 bg-white/95 p-7 shadow-card sm:p-9">
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2">
          <div className="rounded-[1.5rem] bg-mist p-5">
            <p className="text-xs normal-case tracking-[0.16em] text-ink/50">
              {copy.publicPotSuccess.potLabel}
            </p>
          ...
          <div className="rounded-[1.5rem] bg-white p-5 ring-1 ring-ink/8">
            <p className="text-xs normal-case tracking-[0.16em] text-ink/50">
              {copy.publicPotSuccess.amountLabel}
            </p>
            <p className="mt-2 text-lg font-semibold text-ink">
              {contributionAmount !== null
                ? formatCurrency(contributionAmount, contributionCurrency)
                : copy.publicPotSuccess.unavailableAmount}
            </p>
            <p className="mt-2 text-sm text-ink/65">
        ...
        {sessionId ? (
          <div className="mt-4 rounded-[1.5rem] border border-ink/10 bg-white p-5">
            <p className="text-xs normal-case tracking-[0.16em] text-ink/50">
              {copy.publicPotSuccess.stripeReference}
            </p>
            <p className="mt-3 break-all text-sm leading-6 text-ink/78">
              {sessionId}
            </p>
          </div>
        ) : null}
